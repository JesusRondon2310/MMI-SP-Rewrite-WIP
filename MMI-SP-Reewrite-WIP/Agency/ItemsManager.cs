...
namespace MMI_SP.Agency
{
    ...
    {
        // Modelos disponibles (comentario original resumido)
        // Food, Devices, Papers, Rare..
        internal enum CollectionType { Normal, Midday, Night, Empty };

        /// <summary>
        /// Conjunto de props con un tipo (Normal, Midday, Night).
        /// </summary>
        internal class OfficeItemsCollection : List<OfficeItem>
        {
            ...
            // BLOQUE 2: Funciones
            // ==========================================
            public Result<bool> InitAll()
            {
                foreach (OfficeItem item in this)
                {
                    var result = item.Init();
                    if (result is Err<Prop> err) return new Err<bool>(err.Message);
                }
            ...
            public Result<bool> DeleteItemAt(int i)
            {
                if (i < 0 || i >= Count) return new Err<bool>("Índice fuera de rango.");
                if (this[i].prop == null || !this[i].prop.Exists()) return new Err<bool>("El prop no existe.");
                this[i].prop.Delete();
                RemoveAt(i);
                return new Ok<bool>(true);
            }
        ...
        /// <summary>
        /// Un prop individual con su modelo, posición y rotación.
        /// </summary>
            ...
            public Result<Prop> Init()
            {
                for (int i = 0; i < 5; i++)
                {
                    ...
                    if (prop != null && prop.Exists())
                    {
                        ...
                        return new Ok<Prop>(prop);
                    }
                }
                Logger.Error("Error: OfficeItem Init - prop is null!");
                return new Err<Prop>($"No se pudo crear el prop {modelName}.");
            }
        }

        // ==========================================
        // BLOQUE 5: Colecciones estáticas
        // ==========================================
