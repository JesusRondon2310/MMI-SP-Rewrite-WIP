...
namespace MMI_SP.DB
{
    internal static class VehicleDataBuilder
    {
        ...
        internal static Result<VehicleData> CreateFrom(Vehicle veh, string id)
        {
            ...
            // Refuerzos para mods que a veces fallan en GET_VEHICLE_MOD
            ...
            if (!mods.ContainsKey(10) && Function.Call<bool>(Hash.GET_VEHICLE_MOD_VARIATION, veh, 10))
                mods[10] = 1; // Toldo (roof)

            // Detectar llantas antibalas
            bool canBurst = Function.Call<bool>(Hash.GET_VEHICLE_TYRES_CAN_BURST, veh);
            bool bulletproofTires = !canBurst;

            // Detectar neumáticos personalizados
