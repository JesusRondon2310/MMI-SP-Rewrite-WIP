...
namespace MMI_SP
{
    ...
    {
        /// <summary>
        /// Delegados que definen la forma de los eventos internos del seguro.
        /// Se usan para notificar a otras partes del mod (menú, diálogos, etc.)
        /// cuando un vehículo se asegura, se cancela o se recupera.
        /// </summary>
        /// Delegados
        ...
        /// Se usa para calcular cuánto cobrar al asegurar, recuperar o (en el futuro) quizás otras operaciones.
        /// </summary>
        public enum Multiplier { Insurance, Recover };

        /// <summary>
        /// Tipos de nodo de spawn. Define qué categoría de vehículo se va a recuperar
        /// y así elegir la lista de posiciones adecuada (coche, helicóptero, avión, barco).
        /// </summary>
        public enum SpawnNode { Vehicle, Helicopter, Plane, Boat };

        // Cooordenadas ////////////////////////////////////////////////////////////////////
        // Coches, motos, todoterrenos normales
        ...
        // Vehículos largos (limusinas, camiones)
        ...
        // Vehículos militares (Rhino, Barracks…)
        ...
        // Database file ////////////////////////////////////////////////////////////////////
        // Ruta del archivo XML donde se guardan los vehículos asegurados
        private readonly static string _dbFilePath = AppDomain.CurrentDomain.BaseDirectory + "\\MMI\\db.xml";
        private static XDocument _dbFile; // Cambiado a static y XDocument

        // --- Eventos internos del seguro --- \\

        /// <summary>
        /// Se activa al asegurar un vehículo. Pasa el vehículo recién asegurado.
        /// Out: vehicle
        /// </summary>
        ...
        /// <summary>
        /// Se activa al cancelar un seguro. Pasa el identificador del vehículo (dueño+placa+modelo).
        /// Out: Vehicle
        /// </summary>
        ...
        /// <summary>
        /// Se activa al recuperar un vehículo. Pasa el vehículo spawneado y su marcador en el mapa.
        /// out: Vehicle + Blip
        /// </summary>
        ...
        private static void CreateDBFile()
        {
            ...
            _dbFile = doc; // Así la memoria ya tiene el archivo listo para usarse
        }
        /// Creates the database file.

        // Guarda la base de datos XML en disco. Se llama tras cada cambio (asegurar, cancelar, recuperar). \\
        public static void SaveDBFile() // Cambiado a public y static
        {
            try
            {
                ...
                if (_dbFile != null)
                {
                    ...
                    // MENSAJE EN PANTALLA (Encima del mapa)
                    Utils.ShowNotification("CHAR_CARSITE", "Mors Mutual", "Seguros", "Base de datos ~g~actualizada~s~.");
            ...
            {
                Logger.Info("Error: InsuranceManager - Fallo al guardar DB: " + e.Message);
            }
        }
        // Guarda la base de datos XML en disco. Se llama tras cada cambio (asegurar, cancelar, recuperar). \\
        ...
        // Cambia la matrícula de un vehículo en la BD. MUY ANIDADO. 
        // Se eliminará junto con el sistema XML. No merece la pena refactorizar.
            ...
            if (_dbFile.Element("Vehicles") != null)
            {
                ...
                if (vehicleSection != null)
                {
                    try
                    {
                        ...
                        string modelClassName = Function.Call<string>((Hash)0x7B5280EBA9840C72, "VEH_CLASS_" + modelClass.ToString());
                        ...
                        string modelName = Function.Call<string>((Hash)0x7B5280EBA9840C72, model);
        ...
        /// <param name="vehIdentifier"></param>
        /// <returns></returns>
        public static string GetVehicleOwner(string vehIdentifier)
        {
            string owner = "";
        ...
        {
            if (veh.IsAlive)
                if (!veh.Model.IsTrain && (veh.Model.IsCar || veh.Model.IsBike || veh.Model.IsQuadBike || veh.Model.IsHelicopter || veh.Model.IsPlane))
                    return true;
                else
                    return false;
            else
        ...
        public static string GetVehicleFriendlyName(Vehicle veh, bool showClassName = true)
        {
            if (veh != null && veh.Exists())
            {
                ...
                // 1. Obtenemos la clase (Sedán, Súper, etc.)
                int modelClass = Function.Call<int>(Hash.GET_VEHICLE_CLASS, veh);
                string modelClassName = Function.Call<string>((Hash)0x7B5280EBA9840C72, "VEH_CLASS_" + modelClass.ToString());

                // 2. Obtenemos el nombre del modelo (Zentorno, etc.)
                string modelCode = Function.Call<string>(Hash.GET_DISPLAY_NAME_FROM_VEHICLE_MODEL, veh.Model.Hash);
                string modelName = Function.Call<string>((Hash)0x7B5280EBA9840C72, modelCode);

                // 3. Formateamos el nombre usando .Mods.LicensePlate para la matrícula
                if (showClassName)
                    friendlyname = modelName + " - " + veh.Mods.LicensePlate + " (" + modelClassName + ")";
                else
                    friendlyname = modelName + " - " + veh.Mods.LicensePlate;
        ...
        private XElement GenerateVehicleSection(Vehicle veh)
        {
            ...
            // Reemplazo para SE.Player.GetCurrentCharacterName
            string characterName = "Player";
            if (Game.Player.Character.Model == PedHash.Michael) characterName = "Michael";
            else if (Game.Player.Character.Model == PedHash.Franklin) characterName = "Franklin";
            else if (Game.Player.Character.Model == PedHash.Trevor) characterName = "Trevor";
            generalSection.Add(new XElement("Owner", characterName));
            ...
            if (Function.Call<int>(Hash.GET_NUM_MOD_KITS, veh) != 0)
            {
                ...
                {
                    XElement modElem = new XElement("Mod", new XAttribute("Name", modType));
                    modElem.SetValue(veh.Mods[modType].Index);
                ...
                {
                    XElement modElem = new XElement("ToggleMod", new XAttribute("Name", toggleType));

                    // En v3 usamos el indexador de Mods con el tipo de toggle
                    // .IsInstalled devuelve el booleano que necesitas
                    modElem.SetValue(veh.Mods[toggleType].IsInstalled);
            ...
            // Todas estas propiedades ahora cuelgan de veh.Mods
            ...
            colorsSection.Add(new XElement("PrimaryColor", (int)veh.Mods.PrimaryColor));
            colorsSection.Add(new XElement("SecondaryColor", (int)veh.Mods.SecondaryColor));
            colorsSection.Add(new XElement("PearlescentColor", (int)veh.Mods.PearlescentColor));
            colorsSection.Add(new XElement("RimColor", (int)veh.Mods.RimColor));
            ...
            // Colores de interior
            colorsSection.Add(new XElement("DashboardColor", (int)veh.Mods.DashboardColor));
            colorsSection.Add(new XElement("TrimColor", (int)veh.Mods.TrimColor));
            ...
            int livery2 = veh.Mods[VehicleModType.Livery].Index;
            if (livery2 > 0)
            {
                XElement livery2Section = new XElement("Livery2");
                livery2Section.Add(new XElement("ID", livery2));
                ...
                if (vehSection != null)
                {
                    ...
                    if (currentSection != null)
                        if (currentSection.Element("NumberPlateType") != null)
                            currentSection.Element("NumberPlateType").SetValue((int)veh.Mods.LicensePlateType);
                        else
                    ...
                    if (currentSection != null)
                    {
                        ...
                        foreach (VehicleModType modType in Enum.GetValues(typeof(VehicleModType)))
                        {
                            ...
                            // En v3 usamos el indexador .Mods[tipo].Index
                            modElem.SetValue(veh.Mods[modType].Index);
                        ...
                        foreach (VehicleToggleModType toggleType in Enum.GetValues(typeof(VehicleToggleModType)))
                        {
                            ...
                            // En v3 usamos .IsInstalled directamente
                            modElem.SetValue(veh.Mods[toggleType].IsInstalled);
                    ...
                    // Reemplazamos SE.Vehicle.GetVehicleLivery2(veh) por el índice del mod de livery (Mod Type 48)
                    int livery2Index = veh.Mods[VehicleModType.Livery].Index;
                    ...
                    if (livery2Index > 0)
                    {
                        ...
                        {
                            if (currentSection.Element("ID") != null)
                                currentSection.Element("ID").SetValue(livery2Index);
                            else
            ...
            try
            {
                if (_dbFile.Element("Vehicles") != null)
                    if (_dbFile.Element("Vehicles").Element(vehIdentifier) != null)
                    {
                        ...
                        {
                            // En v3 usamos veh.Mods.LicensePlate y veh.Mods.LicensePlateStyle
                        ...
                        if (vehSection.Element("Mods") != null)
                        {
                            ...
                            if (vehSection.Element("Mods").Elements("Mod") != null)
                            {
                                foreach (XElement mod in vehSection.Element("Mods").Elements("Mod"))
                                {
                                    ...
                                    if (modType == VehicleModType.FrontWheel) // En singular
                                        variation = bool.Parse(vehSection.Element("Mods").Element("FrontTiresCustom").Value);
                                    else if (modType == VehicleModType.RearWheel) // En singular
                            ...
                            if (vehSection.Element("Mods").Elements("ToggleMod") != null)
                            {
                                ...
                                {
                                    // Cambiamos VehicleToggleMod por VehicleToggleModType
                                    VehicleToggleModType toggleType = (VehicleToggleModType)Enum.Parse(typeof(VehicleToggleModType), toggleMod.Attribute("Name").Value);
                                    bool enabled = bool.Parse(toggleMod.Value);

                                    // Reemplazamos veh.ToggleMod(modType, enabled) por acceso directo a .IsInstalled
                                    veh.Mods[toggleType].IsInstalled = enabled;
                                }
                            }
                        }
                        ...
                        // Livery 2 (Reemplazo de SE.Vehicle.SetVehicleLivery2)
                        if (vehSection.Element("Livery2") != null)
                        {
                            int livery2Index = Int32.Parse(vehSection.Element("Livery2").Element("ID").Value);

                            // En v3, la segunda livery se aplica como el Mod de tipo Livery
                            veh.Mods[VehicleModType.Livery].Index = livery2Index;
                        }
